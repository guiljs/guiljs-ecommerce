...
class User extends Model
{
    ...
    public function save()
    {
        ...
        $results = $sql->select("CALL sp_users_save(:desperson, :deslogin, :despassword, :desemail, :nrphone, :inadmin)", array(
            "desperson" => ut8_decode($this->getdesperson()),
    ...
    //             $link = "http://www.guiljs-ecommerce.com.br/admin/forgot/reset?code=$code";

    //             $mailer = new \Mailer($data["desemail"], $data["desperson"], "Redefinir senha do Guiljs e-Commerce", "forgot", array(
    //                 "name" => $data["desperson"],
    //                 "link" => $link,
    //             ));
     ...
     ", array(
        ...
        if (count($results) === 0) {
            ...
            if (count($results2) === 0) {
                ...
                if ($inadmin === true) {
                    $link = "http://www.guiljs-ecommerce.com.br/admin/forgot/reset?code=$result";
                } else {
                    $link = "http://www.guiljs-ecommerce.com.br/forgot/reset?code=$result";
                }
                $mailer = new Mailer($data['desemail'], $data['desperson'], "Redefinir senha de Guiljs e-Commerce", "forgot", array(
                    "name" => $data['desperson'],
                    "link" => $link,
                ));
                $mailer->send();
